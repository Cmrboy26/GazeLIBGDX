...
public class WheatSeeds extends SeedItem {
	
	public WheatSeeds(int size) {
		super(ItemType.WHEAT_SEEDS, size);
	}

	@Override
	protected void draw(Gaze game, Batch batch, float x, float y, float width, float height) {
		batch.draw(game.getSprite("wheatSeeds"), x, y, width, height);
	}

	@Override
	public Item readItem(DataInputStream input, ItemType type, int size) throws IOException {
		return new WheatSeeds(size);
	}
